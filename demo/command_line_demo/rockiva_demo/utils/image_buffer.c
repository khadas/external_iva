...
static int CreateImageBuffer(_ImageBufferManager* m, int width, int height, int channel, int buf_type)
{
    for (int i = 0; i < m->count; i++) {
        ...
        img_buf->index = i;
        printf("create image buffer %d width=%d height=%d channel=%d size=%d fd=%d virt_addr=%p\n", i, width, height,
               channel, size, img_buf->image.dataFd, img_buf->image.dataAddr);
    }
    return 0;
}
...
static int DestroyImageBuffer(_ImageBufferManager* m)
{
    ...
    for (int i = 0; i < m->count; i++) {
        IvaImageBuf* img_buf = &(m->buffer[i]);
        if (img_buf->type == 1) {
            int ret = rk_dma_heap_release(img_buf->image.dataFd, img_buf->image.dataAddr, img_buf->image.size);
            if (ret != 0) {
...
ImageBufferManager CreateImageBufferManagerPreAlloc(int count, int w, int h, int c, int buf_type)
{
